...
void follow_line(){
   ...
   for (i = 0; i < 5; i++ )
   {
	if (data[i]<average_sensor[i]) digital[i]=1;
	else digital[i]=0;
   }
...
int main(void)
{
  ...
  HAL_ADC_Start_DMA(&hadc1, (uint32_t*)data, 5);//Bat DMA
  HAL_TIM_PWM_Start(&htim1,TIM_CHANNEL_1);//Bat PWM cho TIMER1 Kenh1
  HAL_TIM_PWM_Start(&htim1,TIM_CHANNEL_2);//Bat PWM cho TIMER1 Kenh1
  tien();
